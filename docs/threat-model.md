...
- **B4 — OS enforcement boundary:** the experimental Linux x86_64 Bubblewrap process path supplies a deny-network namespace, restricted mount view, sealed seccomp deny policy, and optional Task cgroup; complete adapters must still bind policy to authorized file descriptors, approved destinations, and Task-derived resource limits.
...
| TM-009 | Elevation | Child process inherits daemon socket, credentials, network, unrelated descriptors, or unnecessary kernel interfaces. | Direct mode clears environment and standard streams. Experimental Linux x86_64 Bubblewrap mode uses an isolated root view, no host network, a sealed seccomp deny policy, and preserves no descriptor except the policy FD consumed before Tool execution. | Make isolation mandatory for every untrusted runner; derive mounts from Task Capabilities; replace host-path checks with descriptor binding; evaluate workload-specific syscall allowlists. | Partially mitigated in Bubblewrap boundary; release blocker |
