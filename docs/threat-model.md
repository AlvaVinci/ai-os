...
- future model adapters, the experimental create-new Filesystem and IP-only Network Adapters, and out-of-process Tool adapters.
...
- **B4 — OS enforcement boundary:** the experimental Linux x86_64 Bubblewrap process path supplies a deny-network namespace, restricted mount view, sealed seccomp deny policy, Task-derived cgroup limits, and optional descriptor-bound Task read mounts. The separate Filesystem Adapter supplies descriptor-relative create-new writes without read access. The separate Network Adapter supplies bounded direct TCP exchanges to explicit IP addresses. Complete Process write semantics, general filesystem operations, hostname/TLS networking, and destination-scoped Process networking remain open.
...
| --- | --- | --- | --- | --- | --- |
| TM-001 | Elevation | Prompt injection attempts to add Capabilities or change approval rules. | Strict `TaskSpec`; opaque bounded model decisions; trusted Tool and Network Catalogs; deterministic policy; model output is not deserialized as policy. The Agent advertises only granted Tool routes and supported exact Network destinations. | Concrete inference adapters must parse backend output only through validated decision constructors. | Mitigated for Agent contract |
...
| TM-004 | Tampering | An operation supplies a benign scope but changes high-impact arguments or bytes after approval. | `ExecutionGate` retains the complete operation; Tool and Network operation fields are private; Agent approval dispatch retains the owning adapter kind. | Concrete adapters must cover every security-relevant argument in the retained value. | Mitigated for gate |
...
| TM-007 | Tampering | Filesystem path passes lexical policy but resolves through a symlink, mount change, or race. | Lexical normalization rejects traversal and prefix siblings. The optional Linux Process read-mount path resolves beneath an opened root and mounts the retained descriptor through Bubblewrap. The create-new Filesystem Adapter writes from a retained root descriptor with restrictive `openat2` flags and never overwrites an existing destination. | Extend opened-object enforcement to Process writes and every remaining filesystem operation; protect trusted source roots and define cleanup semantics. | Partially mitigated for Process reads and create-new writes; release blocker |
| TM-008 | Tampering | Allowed hostname resolves or redirects to a different destination. | Exact validated TCP host-and-port matching and network deny by default. The IP-only Network Adapter connects to one `SocketAddr`, verifies the peer, and performs no DNS, proxy, redirect, or TLS interpretation. Agent proposals are reconstructed through the same private catalog and gate; unsupported hostname Capabilities are not advertised. | Define bounded DNS, address-class, TLS-identity, proxy, redirect, and multi-address semantics before enabling hostnames; add destination-scoped Process networking. | Partially mitigated for explicit-IP exchanges; release blocker |
| TM-009 | Elevation | Child process inherits daemon socket, credentials, network, unrelated descriptors, or unnecessary kernel interfaces. | Direct mode clears environment and standard streams. Experimental Linux x86_64 Bubblewrap mode uses an isolated root view, no host network, a sealed seccomp deny policy, and a private broker whose explicit policy and read-Capability descriptors are consumed before Tool execution. | Make isolation mandatory for every untrusted runner; bind rootfs, scratch, and executable use to descriptors; evaluate workload-specific syscall allowlists. | Partially mitigated in Bubblewrap boundary; release blocker |
...
| TM-011 | Denial of service | Oversized frames, Tasks, approvals, model turns, Tool arguments, outputs, retained collections, or exposed kernel subsystems exhaust resources. | Explicit limits on frames, Tasks, Events, approvals, model turns, final output, routes, arguments, and Tool output. Task-bound cgroup v2 execution derives cumulative CPU time, resident memory, and wall time from the stable Budget, terminates the tree, and records `BUDGET_EXCEEDED`. Filesystem and Network operations retain the original Task context across approval; network socket stages use the remaining wall-time. The Linux x86_64 seccomp policy blocks `io_uring`, performance events, userfault handling, and other unnecessary kernel surfaces. | Disk, process count, concrete model context, connection rate, decompression, model inference, GPU, and VRAM limits remain. Synchronous filesystem I/O is not preemptible. | Partially mitigated |
...
| TM-013 | Tampering | Runtime restart loses pending approvals or complete operations but persisted state implies resumability, while stale Tool processes continue outside the recovered control plane. | Socket-path exclusion is established before recovery; before accepting requests, recovery records `RUNTIME_RESTARTED` and `failed` for every interrupted Task. When configured with an exclusive delegated root, the daemon then selects only cgroups named from recovered Task IDs, kills their process trees, verifies empty state and identity, and removes them without recursive traversal. No input, operation, or approval authority is reconstructed; audit or cleanup failure aborts startup, and repeat recovery is idempotent. | Database and delegated-root ownership across different socket paths is documented but not locked. Cgroups missing from durable Events and Task scratch content require operator-reviewed cleanup. Any future resumable design needs encrypted retention, key management, fresh authorization, and replay controls. | Partially mitigated for v0.1 non-resumable contract |
...
| TM-016 | Denial of service | A Tool or resource adapter blocks forever or returns after partial side effects. | Input/output bounds, one execution attempt through the gate, original Task wall-time retention across Filesystem and Network approval, Task-bounded synchronous network stages, atomic Agent Network Budget failure, Bubblewrap descendant containment, and complete-cgroup resource or timeout termination. | General asynchronous cancellation, preemptible filesystem I/O, idempotency declaration, non-Process Tool budgets, and additional descendant escape variants remain. | Partially mitigated |
...
| TM-020 | Elevation | Dependency or build compromise changes policy or adapter behavior. | Small Rust dependency set, lockfile, forbidden unsafe code in workspace crates. The seccomp descriptor uses the safe, feature-limited `rustix` filesystem API. | Add dependency audit, provenance, release signing, and reproducible build checks. | Open |

## Abuse cases

### Untrusted content requests network exfiltration

The model may propose egress after reading malicious content. The Agent exposes only supported Task-granted destinations and reconstructs the complete proposal through its private Network Catalog. Network deny-by-default blocks the request unless the Task already grants the exact TCP host and port. If egress also requires approval, the complete destination and request remain paused. Approval cannot add or alter a destination or reset the Task wall-time. The IP-only Network Adapter binds an explicit address to the actual peer, bounds socket stages by the remaining Task wall-time, and performs no DNS, proxy, redirect, or TLS interpretation. Future hostname and HTTPS support must enforce those behaviors explicitly.
...
| SEC-007 | capacity tests across API, Event, approval, gate, and Tool Adapter; atomic `BUDGET_EXCEEDED` tests; Linux Agent-to-cgroup Task Budget tests for CPU time, wall time, and resident memory; Filesystem and Network approval-deadline tests; stalled-network wall-time and Agent terminal Budget tests | preemptible filesystem I/O, disk, process-count, model-context, connection-rate, model-inference, GPU, and VRAM evidence |
| SEC-008 / SEC-009 | private in-process adapters, descriptor-relative create-new write tests, IP-bound bounded TCP exchange and Agent routing tests, content-addressed sealed rootfs, fresh Task-ID-scoped scratch and cgroup tests, Bubblewrap launch-plan tests, sealed seccomp policy unit tests, and a Linux x86_64 boundary suite for root/scratch visibility, descriptor closure, control-socket invisibility, host-network denial, syscall denial, resource termination, descendant cleanup, recovered-ID-scoped stale-cgroup cleanup, and descriptor-bound Task read mounts across path replacement | hostname/TLS and destination-scoped Process network enforcement, Filesystem Agent routing, write-only Process enforcement and remaining filesystem operations, OS-backed rootfs immutability, cross-daemon root ownership, and workload-specific syscall allowlist evidence |
