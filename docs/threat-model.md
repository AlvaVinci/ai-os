...
| TM-009 | Elevation | Child process inherits daemon socket, credentials, network, or unrelated descriptors. | Process handler clears the environment and standard streams and uses a fixed executable; non-standard descriptors remain ambient. | Separate OS principal or sandbox, close-on-exec descriptors, explicit descriptor allowlist, no control socket. | Release blocker |
