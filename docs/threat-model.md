...
- **B4 — OS enforcement boundary:** future isolated adapters must bind policy to actual file descriptors, destinations, processes, and resource limits.
...
| --- | --- | --- | --- | --- | --- |
| TM-001 | Elevation | Prompt injection attempts to add Capabilities or change approval rules. | Strict `TaskSpec`; deterministic policy; model output is not deserialized as policy. | Future planners must keep generated operations separate from Task authority. | Mitigated in core |
| TM-002 | Elevation | An AI runner calls an approval endpoint and approves its own operation. | Protocol Version 2 exposes no approval method. | Approval API requires a separate principal and control channel; see [ADR-0001](adr/0001-separate-approval-principal.md). | Release blocker |
...
| TM-008 | Tampering | Allowed hostname resolves or redirects to a different destination. | Exact validated host matching; network deny by default. | Network adapter must pin actual destinations, control redirects and proxies, and defend against DNS rebinding. | Release blocker |
| TM-009 | Elevation | Child process inherits daemon socket, credentials, network, or unrelated descriptors. | No out-of-process runner exists yet. | Separate OS principal or sandbox, close-on-exec descriptors, clean environment, explicit descriptor allowlist, no control socket. | Release blocker |
...
| TM-011 | Denial of service | Oversized frames, Tasks, approvals, Tool arguments, outputs, or retained collections exhaust memory. | Explicit limits on frames, Tasks, Events, approvals, routes, arguments, and Tool output. | CPU, disk, model context, connection rate, and decompression limits remain. | Partially mitigated |
...
| TM-013 | Tampering | Runtime restart loses pending approvals or complete operations but persisted state implies resumability. | Approval state is process-local; docs explicitly deny resumability. | Recovery must never recreate authority from audit Events alone; encrypted operation storage needs a fresh authorization design. | Open |
...
| TM-016 | Denial of service | A Tool handler blocks forever or returns after partial side effects. | Input/output bounds and one execution attempt through the gate. | Handler timeout, cancellation, idempotency declaration, and isolated termination are not implemented. | Open |
...
| TM-018 | Information disclosure | Another same-user process reads the socket or future encrypted Task key material. | Socket and database use owner-only permissions. | Same-user separation needs sandboxing, peer credentials, separate principals, and keychain or kernel-backed key handling. | Release blocker for approval API |
...
The model may propose egress after reading malicious content. Network deny-by-default blocks the request unless the Task already grants the exact host. If egress also requires approval, the operation remains paused. Approval cannot add a new host. The future network adapter must bind the allowed host to the actual connection destination.
...
| SEC-008 / SEC-009 | private in-process adapters and documented boundary | Linux sandbox, descriptor, filesystem race, and network destination tests |
