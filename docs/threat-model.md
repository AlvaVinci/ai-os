...
| TM-016 | Denial of service | A Tool handler blocks forever or returns after partial side effects. | Input/output bounds, one execution attempt through the gate, Task wall-time propagation, Bubblewrap descendant containment, and complete-cgroup resource or timeout termination. | General asynchronous cancellation, idempotency declaration, non-Process Tool budgets, and additional descendant escape variants remain. | Partially mitigated |
...
| SEC-007 | capacity tests across API, Event, approval, gate, and Tool Adapter; atomic `BUDGET_EXCEEDED` tests; Linux Agent-to-cgroup Task Budget tests for CPU time, wall time, and resident memory | disk, process-count, model-context, connection-rate, model-inference, GPU, and VRAM evidence |
