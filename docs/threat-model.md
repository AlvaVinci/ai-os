...
| TM-007 | Tampering | Filesystem path passes lexical policy but resolves through a symlink, mount change, or race. | Lexical normalization rejects traversal and prefix siblings. The optional Linux Process read-mount path resolves beneath an opened root with restrictive `openat2` flags and mounts the retained descriptor through Bubblewrap. | Extend opened-object enforcement to write semantics and every filesystem adapter operation. | Partially mitigated in read-only Process boundary; release blocker |
