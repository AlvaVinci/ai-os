...
- future model and network adapters, the experimental create-new Filesystem Adapter, and out-of-process Tool adapters.
...
| SEC-008 / SEC-009 | private in-process adapters, descriptor-relative create-new write tests, content-addressed sealed rootfs, fresh Task-ID-scoped scratch and cgroup tests, Bubblewrap launch-plan tests, sealed seccomp policy unit tests, and a Linux x86_64 boundary suite for root/scratch visibility, descriptor closure, control-socket invisibility, host-network denial, syscall denial, resource termination, descendant cleanup, recovered-ID-scoped stale-cgroup cleanup, and descriptor-bound Task read mounts across path replacement | write-only Process enforcement and remaining filesystem operations, OS-backed rootfs immutability, approved network destination tests, cross-daemon root ownership, and workload-specific syscall allowlist evidence |
