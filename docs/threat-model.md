...
| TM-011 | Denial of service | Oversized frames, Tasks, approvals, model turns, Tool arguments, outputs, retained collections, or exposed kernel subsystems exhaust resources. | Explicit limits on frames, Tasks, Events, approvals, model turns, final output, routes, arguments, and Tool output. Optional Task-ID-scoped cgroup v2 execution limits cumulative process CPU time, resident memory, and swap. The Linux x86_64 seccomp policy blocks `io_uring`, performance events, userfault handling, and other unnecessary kernel surfaces. | Wire resource limits to the stable Task Budget and Events; disk, process count, concrete model context, connection rate, decompression, GPU, and VRAM limits remain. | Partially mitigated |
...
| TM-016 | Denial of service | A Tool handler blocks forever or returns after partial side effects. | Input/output bounds, one execution attempt through the gate, a direct-child timeout, Bubblewrap descendant containment, and optional complete-cgroup resource or timeout termination. | General asynchronous cancellation, idempotency declaration, Task Budget/Event integration, and additional descendant escape variants remain. | Partially mitigated |
...
| SEC-007 | capacity tests across API, Event, approval, gate, and Tool Adapter; Linux cgroup CPU-time and resident-memory boundary tests | Task Budget/Event integration plus disk, process-count, model-context, connection-rate, GPU, and VRAM evidence |
