...
| SEC-008 / SEC-009 | private in-process adapters, content-addressed sealed rootfs, fresh Task-ID-scoped scratch and cgroup tests, Bubblewrap launch-plan tests, sealed seccomp policy unit tests, and a Linux x86_64 boundary suite for root/scratch visibility, descriptor closure, control-socket invisibility, host-network denial, syscall denial, resource termination, and descendant cleanup after exit or timeout | OS-backed rootfs immutability, descriptor-bound filesystem race, Capability-derived mount, approved network destination tests, and workload-specific syscall allowlist evidence |
