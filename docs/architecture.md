...
- Optionally launches through an explicit Linux Bubblewrap backend with the verified root filesystem, the Task scratch mounted read-write, namespace isolation, no host network, dropped capabilities, and exact Task read Capabilities mounted from opened descriptors.
