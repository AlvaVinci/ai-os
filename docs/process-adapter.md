...
The separate [Filesystem Adapter](filesystem-adapter.md) now proves a descriptor-relative create-new write can preserve the rule that write does not imply read. It does not grant write authority to an untrusted Process Tool. The next Process increment requires a trusted post-mount enforcement stage or an equivalent kernel boundary before enabling write Capabilities. OS-backed rootfs immutability, safe Task scratch lifecycle, destination-scoped network brokering, model/GPU budgets, cross-daemon ownership locking, and workload-specific syscall allowlists remain required before release claims. See [ADR-0006](adr/0006-bubblewrap-process-isolation.md), [ADR-0007](adr/0007-task-scoped-scratch.md), [ADR-0008](adr/0008-content-addressed-rootfs.md), [ADR-0009](adr/0009-task-cgroup-v2-resource-boundary.md), [ADR-0010](adr/0010-sealed-seccomp-deny-policy.md), [ADR-0011](adr/0011-bind-task-budget-to-process-execution.md), [ADR-0012](adr/0012-reconcile-stale-task-cgroups-on-startup.md), [ADR-0013](adr/0013-descriptor-bound-read-capabilities.md), and [ADR-0014](adr/0014-create-new-write-filesystem-adapter.md).
