...
impl ExecutionAdapter<TcpExchangeOperation> for NetworkAdapter {
    ...
    fn execute(&mut self, operation: TcpExchangeOperation) -> Result<Self::Output, Self::Error> {
        ...
        let mut stream = TcpStream::connect_timeout(&destination, self.timeout)
            .map_err(|_| NetworkAdapterError::ConnectFailed)?;
        stream
            .set_read_timeout(Some(self.timeout))
            .and_then(|()| stream.set_write_timeout(Some(self.timeout)))
            .map_err(|_| NetworkAdapterError::SocketConfigurationFailed)?;
        if stream
            .peer_addr()
        ...
        stream
            .write_all(&operation.request)
            .and_then(|()| stream.shutdown(Shutdown::Write))
            .map_err(|_| NetworkAdapterError::WriteFailed)?;

        let mut bytes = Vec::new();
        stream
            .take((MAX_TCP_RESPONSE_BYTES + 1) as u64)
            .read_to_end(&mut bytes)
            .map_err(|_| NetworkAdapterError::ReadFailed)?;
...
mod tests {
    ...
    fn running_supervisor(
        ...
        let SubmitResult::Accepted(task) = supervisor
            .submit(task(host, port, required_for))
            .expect("submit Task")
        else {
