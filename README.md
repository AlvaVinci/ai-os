...
- experimental Linux Bubblewrap launch plan with a content-addressed sealed minimal rootfs, fresh Task-ID-scoped scratch, namespace isolation, and deny-by-construction host network
...
The `Linux sandbox` workflow additionally runs the ignored Bubblewrap boundary suite on the pinned `ubuntu-24.04` runner image. Local Linux hosts can run the same suite with the command documented in [Process adapter](docs/process-adapter.md#linux-boundary-verification).
